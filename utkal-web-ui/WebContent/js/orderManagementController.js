function getCustomerOrderDetails(){
	...
	$.ajax({
		 ...
		 type: 'GET',
	     url : 'http://localhost:8082/api/order/getOrderDetails/'+customerId+'/'+date,	    
	     crossdomain: true,
	     beforeSend : function(){},
	     complete: function() {
	     },
...
function calculate(){
    ...
    $("#totalBill").text((normalJarRate*normalJarOrder+coldJarRate*coldJarOrder)-(normalFilledJarReturn*coldJarRate+coldFilledJarReturn*coldJarRate));
...
function createOrder(){
	...
	$.ajax({		
		 type: 'POST',
	     url : 'http://localhost:8082/api/order/createOrder',
	     data : jsonData,
